...
var getTimesForInstance = options => {
  request
    .get(
      `${server}/wxtiles/layer/${options.layerId}/instance/${
        options.instanceId
      }/times/`
    )
...
var getTileLayerUrl = ({
  ...
  time = time || 0;
  onSuccess(
    `${server}/wxtiles/tile/${layerId}/${styleId}/${instanceId}/${time}/${level}/{z}/{x}/{y}.png?apikey=${apikey}`
  );
};
...
var getLegendUrl = ({ apikey, layerId, styleId, onSuccess, onError }) => {
  onSuccess(
    `${server}/wxtiles/legend/${layerId}/${styleId}/small/horizontal.png?apikey=${apikey}`
  );
};
